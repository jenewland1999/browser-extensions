...
5. Visit `https://www.npmjs.com/package/react` and confirm destination is
...
3. Select **Load unpacked**.
4. Select this repository directory.
5. Run same navigation check.

### Helium

Helium is Chromium-based. Open its extensions page, enable developer mode, load
this directory unpacked, then run same navigation check. Helium remains beta;
test each release you use.
...
Validation formats all files, lints JavaScript, builds extension archive, then
runs tests. Tests verify permission scope, static rule shape, URL preservation,
settings, omnibox behavior, and rejection of lookalike domains. Also load
unpacked in each target browser: browser performs final schema validation, and
Chromium forks can differ.
...
Inspect archive against file list in `scripts/package.mjs`. Archive is optional
for personal use because browsers load repository directory directly. Do not
add generated JavaScript, runtime dependencies, or minified code.
...
2. Increase `version` in `manifest.json` and `package.json`.
3. Run `pnpm validate`.
4. Reload extension from each browser's extensions page.
